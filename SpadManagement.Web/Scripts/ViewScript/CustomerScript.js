...
function getGridData(pageNumber) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            fillGrid(response.CustomerList, pageNumber, response.PageSize);
...
function fillGrid(Array, pageNumber, pageSize) {
    ...
    for (var i = 1; i <= Array.length; i++) {
        ...
        if (table != null) {
            ...
            cell2.innerHTML = Array[i - 1].FullName;
            cell3.innerHTML = Array[i - 1].ManagerName;
            cell4.innerHTML = Array[i - 1].Mobile;
            cell5.innerHTML = Array[i - 1].City.Title;
            cell6.innerHTML = generateInRowToolbar('Customers', Array[i - 1].Id, ' قرارداد ' + Array[i - 1].Title);
...
function submit() {
    ...
    var cityId = $("select#CityId option").filter(":selected").val();
